								...
								vector<SFMFeature> &sfm_f)
{
	...
	{
		// 要把待求帧i上所有特征点的归一化坐标和3D坐标(l帧坐标系上)都找出来 ，找两帧共同观测
...
*/
	...
	for (int i = 0; i < frame_num; i++)
	{
		...
		c_translation[i][0] = c_Translation[i].x();  // l到i帧的变换
		...
		//因为 l帧是参考系，最新帧的平移也是先验，如果不固定住，原本可观的量会变的不可观。
		if (i == l)
		{
	...
	//shur消元有2大作用，一个是在最小二乘中利用H矩阵稀疏的性质进行加速求解，
	//另一个是在sliding window时求解marg掉老帧后的先验信息矩阵
	//shur消元求解 ,加速
