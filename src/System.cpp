...
void System::PubImageData(double dStampSec, Mat &img)
{
    ...
    last_image_time = dStampSec;
    // frequency control 发布频率控制（不是每来一张图像都要发布，但是都要传入readImage()进行处理），保证每秒钟处理的图像不超过FREQ，此处为每秒10帧
    //判断是否发布此帧？？why
    ...
    trackerData[0].readImage(img, dStampSec); // 获取图像，进行特征点的处理(提取，跟踪)，并保存

    for (unsigned int i = 0;; i++)
    {
        bool completed = false;
        completed |= trackerData[0].updateID(i);   //更新id[],第i个位置存放总共特征点的id
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            ...
            auto &cur_pts = trackerData[i].cur_pts;     //特征点？？
    ...
    while (bStart_backend)
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            for (auto &imu_msg : measurement.first)
            {
                ...
                //两camera帧之间的Imu积分，此时current_time为前一图像帧/前一imu帧
                if (t <= img_t)  //img时间滞后与Imu
                {
                    if (current_time < 0)
                        current_time = t;
                ...
                else   //图像帧在两Imu帧之间的积分，此时current_tiem为上一imu帧
                {
